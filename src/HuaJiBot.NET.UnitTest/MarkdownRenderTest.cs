...
internal class MarkdownRenderTest
{
    ...
    [Test]
    public void Table_CellLineBreak_BecomesSpace() =>
        Assert.That(Render("| a |\n|---|\n| x<br>y |"), Is.EqualTo("a\nx y\n"));

    [Test]
    public void Table_EscapedPipeInCode_IsUnescaped() =>
        Assert.That(Render("| a |\n|---|\n| `c\\|d` |"), Is.EqualTo("a\nc|d\n"));

    [Test]
    public void HtmlInline_LineBreak_BreaksLine() =>
        Assert.That(Render("a<br/>b"), Is.EqualTo("a\nb\n"));

    [Test]
    public void HtmlBlock_StyleContent_IsDropped() =>
        Assert.That(Render("<div>x<style>.y{color:red}</style></div>"), Is.EqualTo("x\n"));
}
